...
public class SamplePreparator {
	...
	/**
	 * Reads in a TSV File containing samples for openBIS registration and their
	 * metadata.
	 * 
	 * @param file
	 *            A TSV File that must contain Identifier, space, project,
	 *            experiment and sample type of each sample and optionally the
	 *            parents of each sample. Additional metadata columns must be
	 *            known to openBIS and fit to the sample type in question.
	 * @param designType
	 * @throws IOException
	 * @throws JAXBException
	 */
	public boolean processTSV(File file, ExperimentalDesignType designType) throws IOException, JAXBException {
		switch (designType) {
		case QBIC:
		...
		if (reader instanceof QBiCDesignReader) {
			...
			projectinfo = new ProjectInfo(qReader.getDescription(), qReader.getSecondaryName(), qReader.isPilot(),
	...
	private void processTSV(List<ISampleBean> samples, Map<String, List<ISampleBean>> sampleToChildrenMap) {
	...
	/**
	 * Returns a useful error text if an error occured while reading the TSV or
	 * null otherwise.
	 * 
	...
	/**
	 * Returns a summary (as list of SampleSummaryBeans) of the sample hierarchy
	 * contained in the processed tsv file.
	 * 
	...
	/**
	 * Returns a nested list structure of all processed samples to be
	 * registered, using TSVSampleBeans. The outer list sorts samples by their
	 * hierarchy (parent-child-structure of openBIS), beginning with the highest
	 * samples.
	 * 
	 * @return Nested list of samples to be registered, sorted by hierarchy
	 *         level and sample type.
	 */
	...
	private void fetchSummary(ArrayList<SampleSummaryBean> summary, ArrayList<ISampleBean> currList, int i,
		...
		summary.add(new SampleSummaryBean(currList.get(0).getType(), sampleContent, Integer.toString(currList.size()),
				pool, isPartOfSplit));
	}
